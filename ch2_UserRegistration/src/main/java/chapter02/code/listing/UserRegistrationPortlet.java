...
public class UserRegistrationPortlet extends GenericPortlet {
    ...
    public void renderForm(RenderRequest request, RenderResponse response)
        ...
        if ("success".equalsIgnoreCase((String) request.getAttribute("actionStatus"))) {
            ...
            getPortletContext().getRequestDispatcher("/success.jsp").include(request, response);
            return;
        }
        // -- create action and render URLs for the registration form.
        // -- Reset hyperlink fires a render request
        // -- and Submit button results in an action request
        ...
        getPortletContext().getRequestDispatcher("/registrationForm.jsp").include(request, response);
    }
    ...
    public void registerUser(ActionRequest request, ActionResponse response)
        ...
        // --set the information entered by the user on the registration form as request attribute.
        // -- NOTE : You can't transfer complex objects from action request to render request using setRenderParameter  method
        User user = new User(request.getParameter("firstName"), request.getParameter("lastName"), email);
        request.setAttribute("user", user);

        //-- if email is not entered, show an error message.
        //-- the message is read from the resource bundle and forwarded to render request as request attribute
    ...
    public void renderPrefs(RenderRequest request, RenderResponse response)
            throws PortletException, IOException {
        getPortletContext().getRequestDispatcher("/preferences.jsp").include(request, response);
    }
    ...
    public void renderHelp(RenderRequest request, RenderResponse response)
            throws PortletException, IOException {
        getPortletContext().getRequestDispatcher("/help.jsp").include(request, response);
    }
